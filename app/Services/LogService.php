...
class LogService
{
    ...
    /**
     * Generic log method used across various controllers
     * 
     * @param string $event
     * @param string $message
     ...
     * @return void
     */
    public function log(string $event, string $message, ?int $userId = null, array $context = []): void
    {
        $level = str_contains(strtolower($event), 'error') || str_contains(strtolower($message), 'failed') ? 'error' : 'info';
        ...
        $this->logSystemEvent($level, $message, $logContext);
    }
}
