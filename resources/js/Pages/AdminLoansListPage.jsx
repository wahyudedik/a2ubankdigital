...
const AdminLoansListPage = () => {
    ...
    const handleSearchChange = (e) => {
        ...
        setSearchTerm(val);
        clearTimeout(window._loanSearchTimer);
        window._loanSearchTimer = setTimeout(() => doSearch(val, statusFilter, 1), 300);
    };

    const handleStatusChange = (e) => {
        const val = e.target.value;
        setStatusFilter(val);
        doSearch(searchTerm, val, 1);
    };

    const handleNextPage = () => { if (pagination && pagination.current_page < pagination.total_pages) doSearch(searchTerm, statusFilter, pagination.current_page + 1); };
    const handlePrevPage = () => { if (pagination && pagination.current_page > 1) doSearch(searchTerm, statusFilter, pagination.current_page - 1); };

    const StatCard = ({ icon, title, value, color }) => (
        <div className="bg-white p-6 rounded-lg shadow flex items-start justify-between">
            <div><p className="text-sm font-medium text-gray-500">{title}</p><p className="text-2xl font-bold text-gray-800 mt-1">{value}</p></div>
    ...
    return (
        <div className="p-4 md:p-6 bg-gray-50 min-h-screen">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                <StatCard icon={<DollarSign size={24} className="text-green-600" />} title="Total Portofolio Aktif" value={formatCurrency(summary?.totalActiveLoans || 0)} color="bg-green-100" />
                <StatCard icon={<FileText size={24} className="text-blue-600" />} title="Jumlah Pinjaman Aktif" value={summary?.activeLoansCount || 0} color="bg-blue-100" />
                <StatCard icon={<AlertTriangle size={24} className="text-red-600" />} title="Pinjaman Menunggak" value={summary?.overdueLoansCount || 0} color="bg-red-100" />
            </div>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {(loans || []).length > 0 ? (loans || []).map(loan => (
                                <tr key={loan.id} className="hover:bg-gray-50">
                                    <td className="px-6 py-4 whitespace-nowrap"><div className="flex items-center"><div className="flex-shrink-0 h-10 w-10 flex items-center justify-center bg-gray-100 rounded-full"><User size={20} className="text-gray-500" /></div><div className="ml-4"><div className="text-sm font-medium text-gray-900">{loan.customer_name}</div><div className="text-sm text-gray-500">ID Pinj: {loan.id}</div></div></div></td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700"><div className="flex items-center"><Tag size={14} className="mr-2 text-gray-400" />{loan.product_name || 'N/A'}</div></td>
                ...
                {pagination && pagination.total_records > 0 && (
                    <div className="flex items-center justify-between border-t border-gray-200 px-4 py-3 sm:px-6">
                        <div className="flex-1 flex justify-between sm:hidden">
                            <button onClick={handlePrevPage} disabled={!pagination || pagination.current_page === 1} className="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50">Sebelumnya</button>
                            <button onClick={handleNextPage} disabled={!pagination || pagination.current_page === pagination.total_pages} className="ml-3 relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50">Berikutnya</button>
                        </div>
                        <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                            <div><p className="text-sm text-gray-700">Halaman <span className="font-medium">{pagination.current_page}</span> dari <span className="font-medium">{pagination.total_pages}</span> ({pagination.total_records} total data)</p></div>
                            <div><nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                                <button onClick={handlePrevPage} disabled={!pagination || pagination.current_page === 1} className="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 disabled:opacity-50"><ChevronLeft className="h-5 w-5" /></button>
                                <button onClick={handleNextPage} disabled={!pagination || pagination.current_page === pagination.total_pages} className="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 disabled:opacity-50"><ChevronRight className="h-5 w-5" /></button>
                            </nav></div>
                        </div>
                    </div>
                )}
