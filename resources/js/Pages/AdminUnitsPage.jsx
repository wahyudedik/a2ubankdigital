...
const StatusBadge = ({ isActive }) => (
    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>{isActive ? 'Aktif' : 'Non-Aktif'}</span>
);
...
const AdminUnitsPage = () => {
    ...
    const handleEdit = (unitOrBranch, isBranch = false) => {
        if (isBranch) { setSelectedUnit({ ...unitOrBranch, parent_id: null }); } else { setSelectedUnit(unitOrBranch); }
        setIsModalOpen(true);
    };
                ...
                {branchList.map(branch => (
                    <div key={branch.id} className="bg-white rounded-lg shadow-md">
                        <div className="p-4 bg-gray-50 border-b flex justify-between items-center rounded-t-lg">
                            <div><h2 className="font-bold text-lg text-gray-800">{branch.unit_name}</h2><p className="text-sm text-gray-500">CABANG</p></div>
                            <div className="flex items-center gap-4"><StatusBadge isActive={branch.is_active} /><button onClick={() => handleEdit(branch, true)} className="text-blue-600 hover:text-blue-800"><Edit size={18} /></button></div>
                        </div>
                        <div className="divide-y">
                            {branch.units?.length > 0 ? branch.units.map(unit => (
                                <div key={unit.id} className="p-4 flex justify-between items-center">
                                    ...
                                    <div className="flex items-center gap-4"><StatusBadge isActive={unit.is_active} /><button onClick={() => handleEdit(unit)} className="text-blue-600 hover:text-blue-800"><Edit size={18} /></button></div>
                                </div>
