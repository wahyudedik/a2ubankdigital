    ...
    return (
        <div>
            ...
            <a href={fullUrl} target="_blank" rel="noopener noreferrer">
                <img src={fullUrl} alt={label} className="mt-2 rounded-lg border w-full object-cover cursor-pointer hover:opacity-80 transition-opacity"
...
const StatusActionCard = ({ customer, onStatusChange }) => {
    ...
    const renderActionButton = () => {
        switch (customer.status) {
            case 'DORMANT':
                return (<button onClick={() => handleChangeStatus('ACTIVE', 'Aktifkan Akun', 'Konfirmasi Aktivasi Akun', `Anda yakin ingin mengaktifkan kembali akun DORMANT untuk nasabah ${customer.full_name}?`)} disabled={loading} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700 disabled:opacity-50">{loading ? 'Memproses...' : 'Aktifkan (Dormant)'}</button>);
            case 'BLOCKED':
                return (<button onClick={() => handleChangeStatus('ACTIVE', 'Buka Blokir', 'Konfirmasi Buka Blokir', `Anda yakin ingin membuka blokir akun untuk nasabah ${customer.full_name}?`)} disabled={loading} className="px-4 py-2 bg-green-600 text-white rounded-lg text-sm font-semibold hover:bg-green-700 disabled:opacity-50">{loading ? 'Memproses...' : 'Buka Blokir'}</button>);
            case 'ACTIVE':
                return (<button onClick={() => handleChangeStatus('BLOCKED', 'Blokir Akun', 'Konfirmasi Blokir Akun', `Anda yakin ingin memblokir akun untuk nasabah ${customer.full_name}?`)} disabled={loading} className="px-4 py-2 bg-red-600 text-white rounded-lg text-sm font-semibold hover:bg-red-700 disabled:opacity-50">{loading ? 'Memproses...' : 'Blokir Akun'}</button>);
            default: return null;
        }
    };
        ...
        <InfoCard title="Status Akun & Tindakan" icon={<ShieldAlert size={20} />}>
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <span className={`mt-1 inline-block px-3 py-1 text-sm font-semibold rounded-full ${currentStatus.color}`}>{currentStatus.text}</span>
                </div>
...
function CustomerDetailPage() {
    ...
    const refreshCustomer = () => {
        router.reload({ only: ['customer'], onSuccess: (page) => { setCustomer(page.props.customer); } });
    };
        ...
        <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-1 space-y-6">
                    ...
                    <StatusActionCard customer={customer} onStatusChange={refreshCustomer} />
