...
const TransactionIcon = ({ type, flow }) => {
    ...
    if (type.includes('TRANSFER') || type.includes('QR')) { if (flow === 'KREDIT') return <ArrowDown {...iconProps} className="text-green-600" />; return <ArrowUp {...iconProps} className="text-red-600" />; }
    switch (type) {
        case 'PEMBELIAN_PRODUK': case 'BAYAR_CICILAN': case 'TARIK_TUNAI': return <ArrowUp {...iconProps} className="text-red-600" />;
        case 'SETOR_TUNAI': case 'PENCAIRAN_PINJAMAN': return <ArrowDown {...iconProps} className="text-green-600" />;
...
const DashboardPage = () => {
    ...
    const chartData = {
        ...
        datasets: [
            { label: 'Pemasukan', data: dashboardData?.weekly_summary?.pemasukan || [], borderColor: '#1E3A8A', backgroundColor: 'rgba(30, 58, 138, 0.1)', tension: 0.4, fill: true },
            { label: 'Pengeluaran', data: dashboardData?.weekly_summary?.pengeluaran || [], borderColor: '#DC2626', backgroundColor: 'rgba(220, 38, 38, 0.1)', tension: 0.4, fill: true }
        ]
    };
    const chartOptions = { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, ticks: { callback: function (value) { if (value >= 1000000) return (value / 1000000).toFixed(1) + ' Jt'; if (value >= 1000) return (value / 1000) + ' Rb'; return value; } } }, x: { grid: { display: false } } }, plugins: { legend: { display: true, position: 'top', align: 'end', labels: { boxWidth: 12, font: { size: 10 } } } } };
