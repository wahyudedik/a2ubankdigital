...
const DepositProductsPage = () => {
    ...
    const handleAdd = () => { setSelectedProduct(null); setIsEditModalOpen(true); };
    const handleEdit = (product) => { setSelectedProduct(product); setIsEditModalOpen(true); };
    const handleSaveSuccess = () => { setIsEditModalOpen(false); router.reload(); };

    const handleDelete = async (productId) => {
        try {
            hideModal();
            const result = await callApi('admin_update_deposit_product.php', 'DELETE', { id: productId });
            ...
            showModal(<AlertDialog title="Error" message={"Gagal menghapus produk. Kemungkinan produk ini masih terkait dengan data pinjaman lain."} onConfirm={hideModal} />);
        }
    };

    const confirmDelete = (productId) => {
        showModal(<AlertDialog title="Konfirmasi Hapus" message="Apakah Anda yakin ingin menghapus produk ini? Tindakan ini tidak dapat dibatalkan." onConfirm={() => handleDelete(productId)} onCancel={hideModal} />);
    };
    ...
    return (
        <div className="p-4 md:p-6 bg-gray-50 min-h-screen">
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-6">
                ...
                <Button onClick={handleAdd} className="w-full sm:w-auto"><PlusCircle className="mr-2 h-4 w-4" /> Tambah Produk</Button>
            </div>
                        ...
                        {loading ? (
                            ...
                            <tr key={p.id} className="hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${p.is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>{p.is_active ? 'Aktif' : 'Non-Aktif'}</span>
                                </td>
            ...
            {isEditModalOpen && <DepositProductModal product={selectedProduct} onClose={() => setIsEditModalOpen(false)} onSave={handleSaveSuccess} />}
        </div>
    );
};
