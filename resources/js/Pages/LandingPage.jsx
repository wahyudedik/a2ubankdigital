    ...
    return (
        <div className="bg-white p-4 rounded-lg border">
            ...
            <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-40 mt-2' : 'max-h-0'}`}>
                <p className="text-gray-600 text-sm pt-2 border-t">{answer}</p>
            </div>
        </div>
    );
};
...
const LandingPage = () => {
    const { appLinks: serverAppLinks } = usePage().props;
    const [isMenuOpen, setIsMenuOpen] = useState(false);
    const [isImageVisible, setIsImageVisible] = useState(false);

    const appLinks = {
        ios: serverAppLinks?.ios || '#',
        android: serverAppLinks?.android || '#'
    };
    ...
    const features = [
        { icon: <BarChart2 className="w-10 h-10 text-bpn-blue-800" />, title: "Prinsip Syariah", description: "Semua produk dan layanan kami dirancang sesuai dengan prinsip keuangan Syariah." },
        { icon: <CreditCard className="w-10 h-10 text-bpn-yellow-500" />, title: "Transaksi Mudah", description: "Nikmati kemudahan transfer, pembayaran, dan pembelian dalam satu genggaman." },
        { icon: <ShieldCheck className="w-10 h-10 text-bpn-red-500" />, title: "Aman & Terpercaya", description: "Keamanan data dan dana Anda adalah prioritas utama kami dengan teknologi terkini." }
    ];

    const faqs = [
        { question: "Bagaimana cara membuka akun?", answer: "Anda dapat membuka akun secara online melalui tombol 'Buka Akun' di atas. Prosesnya cepat, mudah, dan hanya membutuhkan beberapa menit." },
        { question: "Apakah data saya aman?", answer: "Kami menggunakan teknologi enkripsi terdepan dan sistem keamanan berlapis untuk memastikan semua data dan transaksi Anda aman bersama kami." },
        { question: "Apa saja produk yang ditawarkan?", answer: "Kami menawarkan berbagai produk berbasis syariah, mulai dari tabungan, deposito, hingga produk pembiayaan yang sesuai dengan kebutuhan Anda." }
    ];
    ...
    return (
        <div className="bg-gray-50 text-gray-800">
            <header className="container mx-auto px-6 py-4 flex justify-between items-center sticky top-0 bg-white/80 backdrop-blur-md z-20">
                <Link href="/" className="flex items-center"><img src={AppConfig.brand.logo} alt="A2U Bank Digital Logo" className="h-8" /></Link>
                <nav className="hidden md:flex items-center space-x-8 text-gray-600 font-medium"><a href="#features" className="hover:text-bpn-blue-800 transition-colors">Fitur Unggulan</a><a href="#about" className="hover:text-bpn-blue-800 transition-colors">Tentang Kami</a><a href="#faq" className="hover:text-bpn-blue-800 transition-colors">FAQ</a></nav>
                <div className="hidden md:flex items-center space-x-4"><Link href="/login" className="text-gray-600 hover:text-bpn-blue-800 font-bold transition-colors">Login</Link><Link href="/register"><Button>Buka Akun</Button></Link></div>
                <div className="md:hidden"><button onClick={() => setIsMenuOpen(!isMenuOpen)}>{isMenuOpen ? <X size={24} /> : <Menu size={24} />}</button></div>
            </header>
            {isMenuOpen && (<div className="md:hidden bg-white shadow-lg"><nav className="flex flex-col items-center space-y-4 py-4"><a href="#features" onClick={() => setIsMenuOpen(false)}>Fitur</a><a href="#about" onClick={() => setIsMenuOpen(false)}>Tentang Kami</a><a href="#faq" onClick={() => setIsMenuOpen(false)}>FAQ</a><Link href="/login" className="w-full px-4"><Button fullWidth>Login</Button></Link></nav></div>)}
