    ...
    return (
        <div>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                ...
                <Button onClick={handleAdd} className="flex items-center gap-2 px-4 py-2 text-base"><PlusCircle size={20} /><span>Tambah Produk</span></Button>
            </div>
                ...
                {products?.map(p => (
                    <div key={p.id} className="bg-white rounded-lg shadow-md p-4">
                        <div className="flex justify-between items-start">
                            ...
                            <span className={`px-2 py-1 text-xs font-semibold rounded-full ${p.is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>{p.is_active ? 'Aktif' : 'Non-Aktif'}</span>
                        </div>
                    ...
                    <tbody className="divide-y">
                        {products?.map(p => (
                            <tr key={p.id}>
                                ...
                                <td className="p-4"><span className={p.is_active ? 'text-green-600' : 'text-gray-500'}>{p.is_active ? 'Aktif' : 'Non-Aktif'}</span></td>
                                <td className="p-4">
            ...
            {isModalOpen && <LoanProductModal product={selectedProduct} onClose={() => setIsModalOpen(false)} onSave={handleSave} />}
        </div>
    );
};
