...
const WithdrawalAccountsPage = () => {
    ...
    const handleChange = (e) => { const { name, value } = e.target; setNewData(prev => ({ ...prev, [name]: value })); };
    ...
    return (
        <div>
            <Link href="/profile" className="flex items-center gap-2 text-gray-600 hover:text-gray-900 mb-6"><ArrowLeft size={20} /><h1 className="text-2xl font-bold text-gray-800">Rekening Penarikan</h1></Link>
            {!isAddMode && (<Button onClick={() => setAddMode(true)} fullWidth className="mb-4"><PlusCircle size={20} className="mr-2" /> Tambah Rekening Baru</Button>)}
            {isAddMode && (
                <div className="bg-white p-4 rounded-lg shadow-md mb-4">
                    ...
                    <form onSubmit={handleSave} className="space-y-3">
                        ...
                        <div className="flex gap-2 justify-end"><Button type="button" onClick={() => setAddMode(false)} className="bg-gray-200 text-gray-800">Batal</Button><Button type="submit" disabled={loading}>{loading ? 'Menyimpan...' : 'Simpan'}</Button></div>
                    </form>
                </div>
            )}
            <div className="space-y-3">
                {(accounts || []).map(acc => (
                    <div key={acc.id} className="bg-white p-3 rounded-lg shadow-sm flex justify-between items-center">
                        <div><p className="font-bold">{acc.bank_name}</p><p className="text-sm">{acc.account_number}</p><p className="text-xs text-gray-500">{acc.account_name}</p></div>
