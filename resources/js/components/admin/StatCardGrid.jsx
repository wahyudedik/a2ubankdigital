...
const formatCurrency = (amount) => {
    ...
    if (amount >= 1000000000) return `Rp ${(amount / 1000000000).toFixed(2)} Miliar`;
...
const StatCard = ({ icon, title, value, isLoading }) => (
    <div className="bg-white p-6 rounded-lg shadow-md">
        {isLoading ? (
            <div className="animate-pulse flex space-x-4"><div className="rounded-full bg-gray-200 h-12 w-12"></div><div className="flex-1 space-y-3 py-1"><div className="h-2 bg-gray-200 rounded"></div><div className="h-4 bg-gray-200 rounded w-3/4"></div></div></div>
        ) : (
            <div className="flex items-center">
                <div className={`p-3 rounded-full bg-blue-100 ${AppConfig.theme.textPrimary} mr-4`}>{icon}</div>
...
const StatCardGrid = ({ isLoading, stats }) => {
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {statCardsData.map((card, index) => (<StatCard key={index} icon={card.icon} title={card.title} value={card.value} isLoading={isLoading || !stats} />))}
        </div>
    );
};
