...
const CustomerGrowthChart = () => {
    ...
    if (loading) return <div className="h-80 flex items-center justify-center text-gray-500">Memuat data grafik...</div>;
    if (error) return <div className="h-80 flex items-center justify-center text-red-500">{error}</div>;
    if (!chartData) return <div className="h-80 flex items-center justify-center text-gray-500">Tidak ada data untuk ditampilkan.</div>;

    return <div className="h-80 relative"><Line data={chartData} options={chartOptions} /></div>;
};
