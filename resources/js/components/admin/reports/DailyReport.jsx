...
const DailyReport = ({ dateFilter }) => {
    ...
    useEffect(() => {
        if (dateFilter && dateFilter.end_date) fetchReport(dateFilter.end_date);
                        ...
                        <table className="w-full text-sm">
                            <tbody>
                                {Object.entries(report.details).length > 0 ? Object.entries(report.details).map(([type, data]) => (
                                    <tr key={type}><td className="py-1">{type.replace(/_/g, ' ')}</td><td className="py-1 text-right">{data.count} trx</td><td className="py-1 text-right font-semibold">{formatCurrency(data.amount)}</td></tr>
