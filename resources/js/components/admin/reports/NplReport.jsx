                ...
                <ul className="space-y-2 text-sm max-h-48 overflow-y-auto pr-2">
                    {report.map((item, index) => (
                        <li key={index} className="flex justify-between items-center p-2 rounded-md hover:bg-gray-50">
                            <div><p className="font-semibold">{item.full_name}</p><p className="text-xs text-gray-500">{item.phone_number}</p></div>
                            <div className="text-right"><p className="font-bold text-red-600">{formatCurrency(item.installment_amount)}</p><p className="text-xs text-red-500">{item.overdue_days} hari</p></div>
                        </li>
                    ))}
                </ul>
