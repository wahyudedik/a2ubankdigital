    ...
    return (
        <ReportCard icon={<UserCheck size={24} />} title="Kinerja Teller">
            <div className="flex gap-2 items-end mb-4">
                ...
                <Button onClick={fetchReport} disabled={!selectedTeller || loading}>{loading ? <Loader2 className="animate-spin" /> : 'Cari'}</Button>
            </div>
            ...
            {report && report.summary && (
                report.summary.transaction_count > 0 ? (
                    <div className="space-y-3">
                        <div className="flex justify-between items-center bg-gray-50 p-3 rounded-lg"><span className="font-medium">Total Setoran</span><span className="font-bold">{formatCurrency(report.summary.total_deposit)}</span></div>
                        <div className="flex justify-between items-center bg-gray-50 p-3 rounded-lg"><span className="font-medium">Total Penarikan</span><span className="font-bold">{formatCurrency(report.summary.total_withdrawal)}</span></div>
                        <div className="flex justify-between items-center bg-gray-50 p-3 rounded-lg"><span className="font-medium">Jumlah Transaksi</span><span className="font-bold">{report.summary.transaction_count}</span></div>
                    </div>
