...
const AdminTransactionDetailModal = ({ transaction, isLoading, onClose }) => {
    ...
    const renderDetails = () => {
        if (isLoading) return <div className="text-center p-8"><Loader2 className="animate-spin inline-block" /> Memuat detail...</div>;
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md transform transition-all">
                ...
                <div className="p-4 max-h-[70vh] overflow-y-auto">{renderDetails()}</div>
                <div className="bg-gray-50 p-3 flex justify-end"><button onClick={onClose} className="px-4 py-2 text-sm bg-gray-200 rounded-lg hover:bg-gray-300">Tutup</button></div>
            </div>
        </div>
    );
};
