...
const TopUpRequestDetailModal = ({ request, onClose, onApprove, onReject, status }) => {
    const API_DOMAIN = AppConfig.api.baseUrl.replace('/api', '');
    const fullImageUrl = request?.proof_of_payment_url
        ? (request.proof_of_payment_url.startsWith('http') ? request.proof_of_payment_url : `${API_DOMAIN}${request.proof_of_payment_url}`)
        : 'https://placehold.co/600x400/EEE/31343C?text=Tidak+Ada+Bukti';
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-lg p-6 relative transform transition-transform duration-300 ease-out scale-100">
                ...
                <div><h3 className="font-semibold mb-2 text-gray-700">Bukti Pembayaran:</h3>
                    <a href={fullImageUrl} target="_blank" rel="noopener noreferrer" className="block border rounded-md overflow-hidden">
                        <img src={fullImageUrl} alt="Bukti Pembayaran" className="w-full h-auto max-h-80 object-contain bg-gray-100" onError={(e) => { e.target.onerror = null; e.target.src = "https://placehold.co/600x400/EEE/31343C?text=Gambar+Tidak+Ditemukan"; }} />
                    </a>
                </div>
                <div className="mt-6 flex justify-end gap-4 border-t pt-4">
                    {status === 'PENDING' ? (<><Button onClick={onReject} className="bg-red-600 hover:bg-red-700">Tolak</Button><Button onClick={onApprove}>Setujui</Button></>) : (<Button onClick={onClose} className="bg-gray-200 text-gray-800 hover:bg-gray-300">Tutup</Button>)}
                </div>
            </div>
        </div>
    );
